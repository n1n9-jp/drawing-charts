# 「描いて答える折れ線グラフ」テンプレート実装計画

## Context

NYT「You Draw It」やFlourish「Draw The Line」に触発された、ユーザーがトレンドを予測して描画し、実際のデータと比較するインタラクティブ折れ線グラフをテンプレート化する。`_temp`フォルダにNYTの参考実装（build.js内の`drawLine`関数）があり、そのロジックをD3 v7 + vanilla JSでモダンに再実装する。
...
<svg>
  ...
  <text class="draw-prompt"> <!-- 「線を描いて予測」指示テキスト -->
...
avgRatio < 0.7	低すぎ！	控えめな予測でした
avgRatio > 1.3	高すぎ！	攻めた予測でした
